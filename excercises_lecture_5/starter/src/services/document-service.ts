...
export class DocumentService {
  ...
  listDocuments(): Document[] {
    const docs = this.persistence.readJson<Array<Document | LegacyDocument>>(DOCUMENTS_META);
  ...
  private withMetadata(doc: Document | LegacyDocument): Document {
  ...
  private removeFromIndexMeta(relativePath: string, id: string): void {
    const meta = this.persistence.readJson<Record<string, string[]>>(relativePath);
    if (!meta || !Object.prototype.hasOwnProperty.call(meta, id)) return;
...
type LegacyDocument = Omit<Document, 'metadata'> & {
