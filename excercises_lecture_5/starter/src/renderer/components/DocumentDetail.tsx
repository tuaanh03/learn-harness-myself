...
export function DocumentDetail({ document, onDelete, onIndexed }: Props) {
  ...
  const indexDocument = async () => {
    ...
    try {
      ...
      const nextChunks = await window.knowledgeBase.indexing.chunks(document.id);
      setChunks(nextChunks);
      onIndexed?.(document.id);
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: '13px', color: '#888', marginBottom: '16px' }}>
        ...
        <div>File type: {document.metadata.fileType === 'markdown' ? 'Markdown' : 'Text'}</div>
