...
export function App() {
  ...
  const handleIndexedDocument = useCallback(async (id: string) => {
    try {
      const docs = await window.knowledgeBase.documents.list();
      setDocuments(docs);
      setSelectedDoc(docs.find(doc => doc.id === id) ?? null);
      const status = await window.knowledgeBase.indexing.status();
      setAppStatus(status);
    } catch (err) {
      console.error('Failed to refresh indexed document:', err);
    }
  }, []);
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button
            ...
            style={{
              padding: '6px 14px',
              background: documents.length === 0 ? '#2a3148' : '#533483',
              color: documents.length === 0 ? '#8888aa' : '#fff',
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '20px' }}>
            {showImport ? (
              ...
              <DocumentDetail
                ...
                onIndexed={handleIndexedDocument}
              />
            ) : (
            ...
            {lastResponse && (
              <div style={{
                ...
                <div style={{ marginTop: '8px', fontSize: '12px', color: '#a0a0c0' }}>
