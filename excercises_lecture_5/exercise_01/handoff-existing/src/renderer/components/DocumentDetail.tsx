...
function formatFileType(fileType: Document['metadata']['fileType']): string {
  return fileType === 'markdown' ? 'Markdown' : fileType === 'text' ? 'Text' : 'Unknown';
}
...
export function DocumentDetail({ document, onDelete }: Props) {
  ...
  const indexDocument = async () => {
    ...
    try {
      ...
      const indexedChunks = await window.knowledgeBase.indexing.chunks(document.id);
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: '13px', color: '#888', marginBottom: '16px' }}>
        ...
        <div>File type: {formatFileType(document.metadata.fileType)}</div>
        <div>Imported: {new Date(document.importedAt).toLocaleString()}</div>
        <div>Size: {(document.size / 1024).toFixed(1)} KB</div>
        <div>Words: {document.metadata.wordCount.toLocaleString()}</div>
        <div>Lines: {document.metadata.lineCount.toLocaleString()}</div>
