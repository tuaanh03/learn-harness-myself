...
describe('latest answer confidence', () => {
  it('does not render a confidence label without an answer', () => {
    const markup = renderToStaticMarkup(
      createElement(LatestAnswerConfidence, { confidence: undefined }),
    );
  ...
  it.each([
    ...
    [0.5, 'Medium'],
    [0.49, 'Low'],
  ])('renders %s as %s confidence', (confidence, expectedLabel) => {
    const markup = renderToStaticMarkup(
      createElement(LatestAnswerConfidence, { confidence }),
    );
