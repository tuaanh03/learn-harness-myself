...
4. **Verify the project before proceeding.**
   - On macOS/Linux or Windows with Bash/WSL available, run `bash init.sh`.
   - On Windows PowerShell without WSL, run `npm.cmd run check`, `npm.cmd test`, and `npm.cmd run build`.
   - If `bash init.sh` cannot run because Bash/WSL is unavailable, use the PowerShell commands above; this is not a product build failure.
...
2. `npm.cmd test` passes.
3. `npm.cmd run build` passes.
4. UI-related changes are manually smoke-tested when applicable.
5. The app launches and the window is visible (`npm run dev`).
6. The feature appears in `feature_list.json` with status `"pass"` and evidence.
7. The code respects Electron layer boundaries defined above.
...
When running on Windows PowerShell, use:

1. `npm.cmd run check`
2. `npm.cmd test`
3. `npm.cmd run build`

All three commands must pass before a feature can be marked complete.
