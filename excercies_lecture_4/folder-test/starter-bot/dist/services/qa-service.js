...
class QaService {
    constructor(persistence, indexingService) {
        this.persistence = persistence;
        this.indexingService = indexingService ?? new indexing_service_1.IndexingService(persistence);
    }
    ...
    async ask(question) {
        ...
        if (chunks.length > 0) {
            ...
            const scored = chunks.map(chunk => {
                ...
                const score = questionWords.reduce((acc, word) => acc + (contentLower.includes(word) ? 1 : 0), 0);
                return { chunk, score };
            });
    ...
    /** Get the Q&A history. */
    getHistory() {
        return this.persistence.readJson(QA_HISTORY_FILE) ?? [];
    }
    generateAnswer(question, citations) {
